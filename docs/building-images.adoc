...
* To build Chromium from Debian packages for the native Docker architecture use:
+
[source,bash]
----
...
----
+
Here `126` is the Chromium major version. Supported architectures are `amd64` and `arm64`. This mode resolves matching `chromium` and `chromium-driver` packages from Debian. Without `--debian`, the existing Ubuntu Chromium build behavior is unchanged.
